...
CREATE OR REPLACE VIEW Sospechosos_Reboot AS
SELECT p.*, up.lugar_id, l.name
FROM ubicacion_persona up
JOIN personas p ON p.persona_id = up.persona_id
JOIN lugares l ON l.lugar_id = up.lugar_id
WHERE p.persona_id IN (SELECT persona_id FROM sospechosos) AND l.name = 'Reboot Academy'; -- mirar filtro
...
WHERE p.persona_id IN (SELECT persona_id FROM Sospechosos_Reboot);
  ...
  -- Creamos vista
CREATE OR REPLACE VIEW mochila_nombre AS
SELECT p.nombre, i.*
FROM inventario i
JOIN personas p ON p.inventario_id = i.inventario_id
WHERE p.persona_id IN (SELECT persona_id FROM Sospechosos_Reboot);
...
WHERE p.nombre LIKE '%Rafiki%' OR p.nombre LIKE '%Pedro%' OR p.nombre LIKE '%Paco%' OR p.nombre LIKE '%Carlos%';
...
SELECT SUM(oro_ingresado) total_oro, descripcion_persona
FROM detalle_tienda
WHERE ticket = 1001
GROUP BY descripcion_persona;
